...
exports.author_detail = (req, res, next) => {
  async.parallel({
    author: callback => Author.findById(req.params.id)
                              .exec(callback),
    author_books: callback => Book.find({ 'author': req.params.id }, 'title summary')
                                  .exec(callback)
  }, 
  (error, { author, author_books }) => {
    if (error) { return next(error) }
    if(!author) {
...
exports.author_delete_post = (req, res, next) =>
  async.parallel({
    ...
    author_books: callback => Book.find({ 'author': req.body.authorid }).exec(callback)
  }, (error, results) => {
    if(error) { return next(error); }
    // Success
    if(results.author_books.length > 0) {
      ...
      Author.findByIdAndRemove(req.body.authorid, error => {
        if(error) { return next(error); }
